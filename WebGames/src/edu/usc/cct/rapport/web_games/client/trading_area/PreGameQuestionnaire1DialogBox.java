...
public class PreGameQuestionnaire1DialogBox extends DialogBox  {
	...
	public PreGameQuestionnaire1DialogBox(final EventBus eventBus, final ExperimentConditions experimentConditions) {
		super(false, true);
...
//		AbsolutePanel absolutePanel = new AbsolutePanel();
		...
		if (localName == "en"){
			HTML html;
    		html = new HTML(constants.SV_Question_Immigration_Reform_Bill());
			absolutePanel.add(html, 10, 10);			

			RadioButton radioButtonQuestions1_1  = new RadioButton("SVQuestion1");
			radioButtonQuestions1_1.setHTML(constants.support());
			...
			radioButtonQuestions1_2.setHTML(constants.oppose());
	...
	public void showNextQuestion() {
		if(getAnswers (SVQuestion1).equals("1") || getAnswers (SVQuestion1).equals("2")) {
			...
			HTML htmlb = new HTML(constants.SV_Question_Immigration_Reform_Bill2());
			...
			btnSubmit.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if(!getAnswers (SVQuestion1).equals("0") & !getAnswers (SVQuestion2).equals("0")) {
						hide();
						experimentConditions.setSVValueImmigrationReformBill(Integer.parseInt(getAnswers (SVQuestion1)));
						experimentConditions.setSVValueImmigrationReformBill2(Integer.parseInt(getAnswers (SVQuestion2)));
						final PreGameQuestionnaire2DialogBox dialogBox = new PreGameQuestionnaire2DialogBox(eventBus, experimentConditions);
						dialogBox.center();
					}
			...
			btnSubmit.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if(!getAnswers (SVQuestion1).equals("0")) {
						hide();
						experimentConditions.setSVValueImmigrationReformBill(Integer.parseInt(getAnswers (SVQuestion1)));
						experimentConditions.setSVValueImmigrationReformBill2(0);
						final PreGameQuestionnaire2DialogBox dialogBox = new PreGameQuestionnaire2DialogBox(eventBus, experimentConditions);
						dialogBox.center();
					}
