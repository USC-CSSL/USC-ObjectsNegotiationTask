...
public class PreGameQuestionnaire2DialogBox extends DialogBox  {
	...
	public PreGameQuestionnaire2DialogBox(final EventBus eventBus, final ExperimentConditions experimentConditions) {
		super(false, true);
...
//		AbsolutePanel absolutePanel = new AbsolutePanel();
		...
		if (localName == "en"){
			HTML html;
    		html = new HTML(constants.SV_Question_Gun_Control_Reform_Bill());
	...
	public void showNextQuestion() {
		if(getAnswers (SVQuestion1).equals("1") || getAnswers (SVQuestion1).equals("2")) {
			...
			HTML htmlb = new HTML(constants.SV_Question_Gun_Control_Reform_Bill2());
			...
			btnSubmit.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if(!getAnswers (SVQuestion1).equals("0") & !getAnswers (SVQuestion2).equals("0")) {
						hide();
						experimentConditions.setSVValueGunControlReformBill(Integer.parseInt(getAnswers (SVQuestion1)));
						experimentConditions.setSVValueGunControlReformBill2(Integer.parseInt(getAnswers (SVQuestion2)));
						final PreGameQuestionnaire3DialogBox dialogBox = new PreGameQuestionnaire3DialogBox(eventBus, experimentConditions);
						dialogBox.center();
					}
			...
			btnSubmit.addClickHandler(new ClickHandler() {
				public void onClick(ClickEvent event) {
					if(!getAnswers (SVQuestion1).equals("0")) {
						hide();
						experimentConditions.setSVValueGunControlReformBill(Integer.parseInt(getAnswers (SVQuestion1)));
						experimentConditions.setSVValueGunControlReformBill2(0);
						final PreGameQuestionnaire3DialogBox dialogBox = new PreGameQuestionnaire3DialogBox(eventBus, experimentConditions);
						dialogBox.center();
					}
